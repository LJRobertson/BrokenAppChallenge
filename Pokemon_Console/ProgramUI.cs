...
namespace Pokemon_Console
{
    class ProgramUI
    {
        private PokemonRepository _pokemonRepo = new PokemonRepository();
        public void Run()
        {
            SeedPokemonTeam();
            RunMenu();
        ...
        {
            Pokemon pika = new Pokemon(101, "Pikachu", "Pikachu", 1, PokemonType.Electric, PokemonType.None, "Sand Attack", "Growl", "Quick Attack", "Tackle");
            _pokemonRepo.AddPokemonToTeam(pika);
        }
        private void SeeMyPokemonTeam()
        {
            Console.Clear();
            List<Pokemon> pokemonTeam = _pokemonRepo.GetPokemonTeam();
        ...
        private void UpdateAPokemonMenu()
        {
            ...
            else
            {
                ...
                Console.WriteLine("\n\nEnter the nickname of pokemon you wish to update:");
                string response = Console.ReadLine();
                //UpdateAPokemon(response);

                Console.WriteLine($"Lets update {response}!" +
                    "\n\nEnter the new Pokemon Species Name: ");
                string species = Console.ReadLine();


                Console.WriteLine("New Pokemon Nick Name: ");
                string name = Console.ReadLine();

                Console.WriteLine("New Pokemon Level: ");
                string pokeLevel = Console.ReadLine();
                ...
                {
                    int level = int.Parse(pokeLevel);
                    Console.WriteLine("New Pokemon Type: " +
                    "\n1. Normal" +
                    "\n2. Grass" +
                    "\n3. Fire" +
                    "\n4. Water" +
                    ...
                    "\n15. Dragon");
                    PokemonType typeOne = (PokemonType)int.Parse(Console.ReadLine());
                    Console.WriteLine("New Pokemon Secondary Type: " +
                        "\n1. Normal" +
                        "\n2. Grass" +
                        "\n3. Fire" +
                        "\n4. Water" +
                    ...
                    Pokemon testPokemon = _pokemonRepo.GetPokemonByTeamPosition();

                    int teamPosition = _pokemonRepo.GetPokemonByTeamPosition(response.TeamPosition);

                    Pokemon newPokemon = new Pokemon(teamPosition, species, name, level, typeOne, typeTwo, moveOne, moveTwo, moveThree, moveFour);
                    _pokemonRepo.UpdatePokemonByTeamPosition(teamPosition, newPokemon);
                    Console.WriteLine("Pokemon updated!");
                    Console.ReadKey();
                    _pokemonRepo.UpdatePokemonByNickName(response, newPokemon);
                }
                Console.Clear();
            }
        }
        ...
        {
            Console.Clear();
            Console.WriteLine("Enter the Team Posiyion for the Pokemon to update:");
            int teamPosition = int.Parse(Console.ReadLine());
            Pokemon pokemon = _pokemonRepo.GetPokemonByTeamPosition(teamPosition);
        ...
        private void RemoveAPokemon()
        {
            ...
            else
            {
                ...
                _pokemonRepo.RemovePokemonFromTeam(response);
                Console.Clear();
